...
const createStartedGameWithXPlayers = (x: number) => {
  return (props: Parameters<typeof createTestGame>[0] = {}) => {
    ...
    const turn = Option.getOrThrow(
      startedGame.toSnapshot().currentTurn,
    );

    return {
      game: startedGame,
      deck,
      storytellerId: turn.currentStorytellerId,
      storytellerHand: Option.getOrThrow(
        Option.fromNullable(
          turn.playerHands.find((hand) =>
            hand.playerId === turn.currentStorytellerId
          ),
        ),
      ),
    };
